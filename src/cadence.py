...
class CadenceMainW(QMainWindow, ui_cadence.Ui_CadenceMainW):
    def __init__(self, parent=None):
        ...
        for iPath in DEFAULT_GIG_PATH:
            self.list_GIG.addItem(iPath)

        for iPath in DEFAULT_SF2_PATH:
            self.list_SF2.addItem(iPath)

        for iPath in DEFAULT_SFZ_PATH:
            self.list_SFZ.addItem(iPath)
        ...
        EXTRA_GIG_DIRS = GlobalSettings.value("AudioPlugins/EXTRA_GIG_PATH", "", type=str)
        EXTRA_SF2_DIRS = GlobalSettings.value("AudioPlugins/EXTRA_SF2_PATH", "", type=str)
        EXTRA_SFZ_DIRS = GlobalSettings.value("AudioPlugins/EXTRA_SFZ_PATH", "", type=str)
        ...
        for iPath in EXTRA_GIG_DIRS.split(":"):
            if os.path.exists(iPath):
                self.list_GIG.addItem(iPath)

        for iPath in EXTRA_SF2_DIRS.split(":"):
            if os.path.exists(iPath):
                self.list_SF2.addItem(iPath)

        for iPath in EXTRA_SFZ_DIRS.split(":"):
            if os.path.exists(iPath):
                self.list_SFZ.addItem(iPath)
        ...
        self.list_GIG.sortItems(Qt.AscendingOrder)
        self.list_SF2.sortItems(Qt.AscendingOrder)
        self.list_SFZ.sortItems(Qt.AscendingOrder)
        ...
        self.list_GIG.setCurrentRow(0)
        self.list_SF2.setCurrentRow(0)
        self.list_SFZ.setCurrentRow(0)
    ...
    def slot_tweaksApply(self):
        if "plugins" in self.settings_changed_types:
            ...
            for i in range(self.list_GIG.count()):
                iPath = self.list_GIG.item(i).text()
                if iPath not in DEFAULT_GIG_PATH and iPath not in EXTRA_GIG_DIRS:
                    EXTRA_GIG_DIRS.append(iPath)

            for i in range(self.list_SF2.count()):
                iPath = self.list_SF2.item(i).text()
                if iPath not in DEFAULT_SF2_PATH and iPath not in EXTRA_SF2_DIRS:
                    EXTRA_SF2_DIRS.append(iPath)

            for i in range(self.list_SFZ.count()):
                iPath = self.list_SFZ.item(i).text()
                if iPath not in DEFAULT_SFZ_PATH and iPath not in EXTRA_SFZ_DIRS:
                    EXTRA_SFZ_DIRS.append(iPath)
            ...
            GlobalSettings.setValue("AudioPlugins/EXTRA_GIG_PATH", ":".join(EXTRA_GIG_DIRS))
            GlobalSettings.setValue("AudioPlugins/EXTRA_SF2_PATH", ":".join(EXTRA_SF2_DIRS))
            GlobalSettings.setValue("AudioPlugins/EXTRA_SFZ_PATH", ":".join(EXTRA_SFZ_DIRS))
    ...
    def slot_tweakPluginAdd(self):
        ...
        if self.tb_tweak_plugins.currentIndex() == 0:
            ...
            self.list_GIG.addItem(newPath)
        elif self.tb_tweak_plugins.currentIndex() == 6:
            self.list_SF2.addItem(newPath)
        elif self.tb_tweak_plugins.currentIndex() == 7:
            self.list_SFZ.addItem(newPath)
    ...
    def slot_tweakPluginChange(self):
        if self.tb_tweak_plugins.currentIndex() == 0:
            ...
            curPath = self.list_GIG.item(self.list_GIG.currentRow()).text()
        elif self.tb_tweak_plugins.currentIndex() == 6:
            curPath = self.list_SF2.item(self.list_SF2.currentRow()).text()
        elif self.tb_tweak_plugins.currentIndex() == 7:
            curPath = self.list_SFZ.item(self.list_SFZ.currentRow()).text()
        else:
            curPath = ""
        ...
        if self.tb_tweak_plugins.currentIndex() == 0:
            ...
            self.list_GIG.item(self.list_GIG.currentRow()).setText(newPath)
        elif self.tb_tweak_plugins.currentIndex() == 6:
            self.list_SF2.item(self.list_SF2.currentRow()).setText(newPath)
        elif self.tb_tweak_plugins.currentIndex() == 7:
            self.list_SFZ.item(self.list_SFZ.currentRow()).setText(newPath)
    ...
    def slot_tweakPluginRemove(self):
        if self.tb_tweak_plugins.currentIndex() == 0:
            ...
            self.list_GIG.takeItem(self.list_GIG.currentRow())
        elif self.tb_tweak_plugins.currentIndex() == 6:
            self.list_SF2.takeItem(self.list_SF2.currentRow())
        elif self.tb_tweak_plugins.currentIndex() == 7:
            self.list_SFZ.takeItem(self.list_SFZ.currentRow())
    ...
    def slot_tweakPluginTypeChanged(self, index):
        ...
        if index == 0:
            ...
            self.list_GIG.setCurrentRow(-1)
            self.list_GIG.setCurrentRow(0)
        elif index == 6:
            self.list_SF2.setCurrentRow(-1)
            self.list_SF2.setCurrentRow(0)
        elif index == 7:
            self.list_SFZ.setCurrentRow(-1)
            self.list_SFZ.setCurrentRow(0)
    ...
    @pyqtSlot(int)
    def slot_tweakPluginsVstRowChanged(self, index):
        nonRemovable = (index >= 0 and self.list_GIG.item(index).text() not in DEFAULT_GIG_PATH)
        self.b_tweak_plugins_change.setEnabled(nonRemovable)
        self.b_tweak_plugins_remove.setEnabled(nonRemovable)

    @pyqtSlot(int)
    def slot_tweakPluginsVstRowChanged(self, index):
        nonRemovable = (index >= 0 and self.list_SF2.item(index).text() not in DEFAULT_SF2_PATH)
        self.b_tweak_plugins_change.setEnabled(nonRemovable)
        self.b_tweak_plugins_remove.setEnabled(nonRemovable)

    @pyqtSlot(int)
    def slot_tweakPluginsVstRowChanged(self, index):
        nonRemovable = (index >= 0 and self.list_SFZ.item(index).text() not in DEFAULT_SFZ_PATH)
        self.b_tweak_plugins_change.setEnabled(nonRemovable)
        self.b_tweak_plugins_remove.setEnabled(nonRemovable)
